...
适用 UI 页面、组件、前端请求、权限渲染、表单、Design Token、紧凑布局、JavaScript、TypeScript、导航和交互开发。

## 细分入口

- `api-and-data.md`：前端请求、列表查询、按需加载和筛选能力。
- `javascript-typescript.md`：JavaScript/TypeScript 类型边界、异步生命周期、状态、依赖和验证规则。
- `form-validation.md`：提交型表单校验、提交拦截、字段错误和服务端业务错误保留。
...
- 说明本次任务涉及哪些前端子域。
- 对复用价值、交互一致性、JS/TS 类型边界、表单校验、颜色 token、紧凑布局、大文件、长函数、过多 props、装饰性文案和悬空路由给出明确处理结论。
