...
- CON-FE-TEST-007 初始渲染验证：涉及异步页面时，应使用可控延迟或未完成请求验证 `state-and-cache.md` 的 `CON-FE-STATE-008`；断言页面壳层先可见、数据区域独立进入 loading，并分别覆盖 success、empty、error 和 retry，避免只测试接口完成后的最终 DOM。
...
- 说明验证关注用户行为、业务结果、可访问入口、初始渲染、UI 稳定性还是内部逻辑，并说明执行结果或无法执行原因。
