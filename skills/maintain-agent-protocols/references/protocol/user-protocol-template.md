...
```text
开发任务      → @playbooks/coding.md
架构设计      → @playbooks/architecture.md
安全审查      → @playbooks/security-review.md
性能审查      → @playbooks/performance-review.md
故障排查      → @playbooks/troubleshooting.md
技术调研      → @playbooks/research.md
```
...
安全审查       → 场景手册的安全审查入口 + 安全工程入口；涉及语言实现时补对应工程入口
性能审查       → 场景手册的性能审查入口 + 性能工程入口；涉及数据访问或可观测性时补核心工程或平台工程入口
架构设计       → 场景手册的架构入口 + 相关工程大类入口
```

## 检查入口

```text
规则维护       → @checks/maintenance-checklist.md
安全检查       → @checks/security-checklist.md
性能检查       → @checks/performance-checklist.md
```
...
| 规范驱动 | 涉及 API、数据结构、配置、权限模型或系统架构时，先确认项目规范是否需要同步。 | 用户协议不承载项目业务规范正文。 |
| 最小修改 | 默认局部修改，避免无关重构、重命名、目录调整和格式化。 | 修复什么，修改什么。 |
| 风险分级 | 认证、授权、数据迁移、删除和安全控制逻辑等高风险变更，需要先说明影响和验证方式。 | 具体执行步骤放入场景手册或项目协议。 |
| 经验优先 | 优先继承项目规范、历史决策、Wiki、设计文档和既有代码模式。 | 不在缺少背景时推翻既有设计。 |
| 结果闭环 | 发现的问题必须归类为已解决、延后处理、明确排除或转入后续任务。 | 禁止问题无故消失。 |
| 持续沉淀 | 只沉淀可复用经验、通用设计模式、架构决策和长期有效规则。 | 一次性问题、临时方案、环境异常和偶发故障不沉淀为长期规则。 |
| Git 谨慎 | 默认不自动提交；仅用户或项目协议明确要求时提交。 | 提交范围仅包含当前任务相关内容。 |
