...
创建或维护用户级、项目级 AI Agent 协作协议时使用本参考。协议文件负责入口和边界，工程路由与场景手册负责执行细节。目标仓统一产物目录和新旧路径兼容规则见仓库共享参考 `target-workspace-layout`。
...
6. 在预览中补充触发稳定性检查：工作流、条件适用路由、项目约束和检查项映射是否存在明显断链；需要强化 playbooks 或 routes 时列为确认项。
...
- 会影响任务触发稳定性的项目级约束，应写明生效条件和验证方式；高风险或高频约束应能通过场景手册、工程路由或检查项短路径触达。
