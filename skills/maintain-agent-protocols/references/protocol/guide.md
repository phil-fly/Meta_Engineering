...
| 项目级 | 仓库专属约束和执行要求 | `AGENTS.md`、`CODEX.md`、`CLAUDE.md`、`ai-agent-protocols/project/AGENTS.md` | 架构、命令、测试、编码约定、发布、业务边界 | 个人偏好、全局哲学、跨项目长流程 |
...
3. 在用户仓库中创建或维护 `ai-agent-protocols/` 目录。
4. 默认生成完整路由骨架，避免遗漏 UI、Go、Java、Rust、通用安全、通用性能和通用质量入口。
5. 用户协议只写入口和原则，不写执行细节。
6. 工程路由写执行细节，并按功能类型互相引用。
7. 检查重复、歧义、冲突、遗漏和意外约束。
